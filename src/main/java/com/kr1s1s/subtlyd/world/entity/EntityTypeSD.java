...
public class EntityTypeSD {
    ...
    private static EntityType<TentEntity> registerTent(DyeColor dyeColor, Item item) {
        return Registry.register(BuiltInRegistries.ENTITY_TYPE, TentEntity.getLocation(dyeColor), EntityType.Builder.of(tentFactory(() -> item, dyeColor), MobCategory.MISC).sized(3.5F, 2.0F).noLootTable().clientTrackingRange(10).build(TentEntity.getResourceKey(dyeColor)));
    }
