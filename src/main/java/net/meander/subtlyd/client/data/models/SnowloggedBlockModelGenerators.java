 ...
 */
public class SnowloggedBlockModelGenerators {
    /**
     ...
     */
    public void generatePlantState(Block block, Identifier blockId, ResourceManager resourceManager) {
        ...
        multipart.add(createConditionPart(BlockStateProperties.SNOWLOGGED_LAYERS.getName(), "0", modelPath, 0, false));
    ...
    public void generateDoublePlantState(Block block, Identifier blockId, ResourceManager resourceManager) {
        if (block != Blocks.PITCHER_CROP && block != Blocks.PITCHER_PLANT) { // Pitcher plants have top and bottom models that are also aging
            ...
            addProperty(bottomDefaultCondition, BlockStateProperties.SNOWLOGGED_LAYERS.getName(), "0");
            ...
            addProperty(topDefaultCondition, BlockStateProperties.SNOWLOGGED_LAYERS.getName(), "0");
            addProperty(topDefaultCondition, BlockStateProperties.BOTTOM_SNOWLOGGED.getName(), "false");
            ...
            addProperty(topSnowyCondition, BlockStateProperties.SNOWLOGGED_LAYERS.getName(), "0");
            addProperty(topSnowyCondition, BlockStateProperties.BOTTOM_SNOWLOGGED.getName(), "true");
    ...
    public void generateAgeableState(Block block, Identifier blockId, ResourceManager resourceManager) {
        Identifier stateId = Identifier.fromNamespaceAndPath(blockId.getNamespace(), "blockstates/" + blockId.getPath() + ".json");
        ...
        if (ageProperty != null) {
            for (Comparable<?> value : ageProperty.getPossibleValues()) {
                ...
                addProperty(defaultCondition, BlockStateProperties.SNOWLOGGED_LAYERS.getName(), "0");
    ...
    private int getAge(Block block, int age) {
        int modelAge = age;
    ...
    public void generateSegmentableState(Block block, Identifier blockId, ResourceManager resourceManager) {
        Identifier stateId = Identifier.fromNamespaceAndPath(blockId.getNamespace(), "blockstates/" + blockId.getPath() + ".json");
    ...
    public void generateFenceState(Block block, Identifier blockId) {
        Identifier stateId = Identifier.fromNamespaceAndPath(blockId.getNamespace(), "blockstates/" + blockId.getPath() + ".json");
        String modelPath = ModelLocationUtils.getModelLocation(block).toString();
    ...
    public void generateAltFenceState(Block block, Identifier blockId) {
        String modelPath = ModelLocationUtils.getModelLocation(block).toString();
        String postModel = modelPath + "_post";
    ...
    public void generateFenceGateState(Block block, Identifier blockId) {
        String modelPath = ModelLocationUtils.getModelLocation(block).toString();
    ...
    public void generateWallState(Block block, Identifier blockId) {
        String modelPath = ModelLocationUtils.getModelLocation(block).toString();
        String postModel = modelPath + "_post";
        String sideModel = modelPath + "_side";
    ...
    public void generateCrossState(Identifier blockId, ResourceManager resourceManager) {
    ...
    private String generateSnowyModel(String targetModelPath, String textureSlot) {
     ...
     */
    private String generateAoModel(String targetModelPath) {
    ...
    private void addUnculledSnowLayers(JsonArray multipart) {
        String layers = BlockStateProperties.SNOWLOGGED_LAYERS.getName();
    ...
    private void generateUnculledSnowModel(int layer, String modelName) {
        int height;
    ...
    private void addSnowLayers(JsonArray multipart) {
        String layers = BlockStateProperties.SNOWLOGGED_LAYERS.getName();
    ...
    private boolean hasSnowyVariant(Identifier modelId, ResourceManager resourceManager) {
     ...
     */
    private JsonObject createMaxLayerCondition(int maxLayer) {
    ...
    private JsonObject createConditionPart(String stateKey, String stateValue, String model, int yRot, boolean uvlock) {
        JsonObject condition = new JsonObject();
    ...
    private JsonObject createPart(JsonObject condition, JsonObject apply) {
    ...
    private JsonObject createApply(String model, int yRot, boolean uvlock) {
     ...
     */
    private void addProperty(JsonObject condition, String key, String value) {
        if (condition.has("OR")) {
